...
def gerar_dados():
    ...
    for nome_arquivo in os.listdir(diretorio_script):
        ...
        #Cria dataframes para cada arquivo cvs
        df = pd.read_csv(os.path.join(diretorio_script, nome_arquivo))
        lista_dataframes.append(df)

        #Percorre a coluna 'ramo_justica' do arquivo
            ...
            if ramo_justica_atual == 'Tribunais Superiores':
                ...
                for sigla_tribunal_atual in df_superiores['sigla_tribunal'].unique():
                    ...
                    if sigla_tribunal_atual == 'STJ':
                        ...
                        print(f"Metas inexistentes para {sigla_tribunal_atual} em {nome_arquivo}")
                        continue

                    #Se não estiver vazio
                    if not df_filtrado_tribunal.empty:
                        print(f"\nCalculando metas para o arquivo: {nome_arquivo}")
